...
// 榜单状态：每个频道的当前分类、页码、选中的书籍
const rankingState = {
  1: { channelId: 1, categoryId: null, page: 1, selected: new Set() },
  0: { channelId: 0, categoryId: null, page: 1, selected: new Set() },
  '-1': { channelId: -1, categoryId: null, page: 1, selected: new Set() },
};

function initRanking() {
  // Tab 切换
  document.querySelectorAll('.ranking-channel-tab').forEach(tab => {
    tab.addEventListener('click', () => {
      const cid = tab.getAttribute('data-channel-id');
      switchRankingChannel(cid);
    });
  });
  ...
  // 频道 1 (男生) 的事件
  setupRankingChannelEvents(1);
  setupRankingChannelEvents(0);
  setupRankingChannelEvents('-1');
}

function switchRankingChannel(cid) {
  document.querySelectorAll('.ranking-channel-tab').forEach(t => {
    t.classList.toggle('active', t.getAttribute('data-channel-id') === cid);
  });
  document.querySelectorAll('.ranking-panel').forEach(p => {
    p.classList.toggle('active', p.getAttribute('data-panel-channel') === cid);
  });
}

async function loadRankingCategories() {
  try {
    const resp = await fetchWithCreds('/api/ranking/categories');
    if (!resp.ok) return;
    const data = await resp.json();
    const channels = data.channels || [];

    channels.forEach(ch => {
      const bar = document.getElementById(`rankingCategoryBar${ch.channel_id}`);
      if (!bar) return;

      const ids = [];
      bar.innerHTML = '<div class="ranking-category-buttons"></div>';
      const btns = bar.querySelector('.ranking-category-buttons');

      ch.ranks.forEach(rank => {
        const btn = document.createElement('button');
        btn.className = 'ranking-category-btn';
        btn.textContent = rank.rank_name;
        btn.setAttribute('data-rank-id', rank.rank_id);
        btn.setAttribute('data-channel-id', ch.channel_id);
        btn.addEventListener('click', () => selectRankingCategory(ch.channel_id, rank.rank_id, btn));
        btns.appendChild(btn);
        ids.push(rank.rank_id);
      });

      // 默认选中第一个
      if (ids.length > 0 && !rankingState[ch.channel_id].categoryId) {
        selectRankingCategory(ch.channel_id, ids[0], btns.children[0]);
      }
    });
  } catch (err) {
    console.error('loadRankingCategories:', err);
  }
}

function selectRankingCategory(channelId, rankId, btn) {
  // 更新按钮状态
  const bar = document.getElementById(`rankingCategoryBar${channelId}`);
  bar.querySelectorAll('.ranking-category-btn').forEach(b => b.classList.remove('active'));
  btn.classList.add('active');

  // 更新状态并加载书籍
  const st = rankingState[channelId];
  st.categoryId = rankId;
  st.page = 1;
  st.selected = new Set();
  updateRankingSelectedCount(channelId);

  loadRankingBooks(channelId);
}

async function loadRankingBooks(channelId) {
  const st = rankingState[channelId];
  if (!st.categoryId) return;

  const tbody = document.getElementById(`rankingBooksBody${channelId}`);
  tbody.innerHTML = '<tr><td colspan="6" class="text-center pad-20">加载中…</td></tr>';

  try {
    const resp = await fetchWithCreds(
      `/api/ranking/books?channel_id=${channelId}&rank_id=${st.categoryId}&page=${st.page}&size=50`
    );
    if (!resp.ok) throw new Error('加载失败');
    const data = await resp.json();

    if (!data.items || data.items.length === 0) {
      tbody.innerHTML = '<tr><td colspan="6" class="text-center pad-20">暂无数据</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = data.items.map(book => {
      const checked = st.selected.has(book.book_id) ? 'checked' : '';
      const desc = book.description ? escHtml(book.description).substring(0, 80) + '…' : '';
      ...
      return `<tr>
        <td class="col-select"><input type="checkbox" class="ranking-book-check"
          data-book-id="${book.book_id}" data-title="${escAttr(title)}"
          data-author="${escAttr(author)}" ${checked} /></td>
        <td>
          <div class="ellipsis" title="${escAttr(title)}">${escHtml(title)}</div>
          ${desc ? `<div class="ranking-desc" title="${escAttr(desc)}">${escHtml(desc)}</div>` : ''}
        </td>
        <td class="ellipsis" title="${escAttr(author)}">${escHtml(author)}</td>
        <td class="k">${escHtml(category)}</td>
        <td class="k">${escHtml(wordCount)}</td>
        <td>
          <button class="sm ranking-download-single" data-book-id="${book.book_id}" data-title="${escAttr(title)}" title="下载本书">
            下载
          </button>
        </td>
      </tr>`;
    ...
    // 绑定 checkbox 事件
    tbody.querySelectorAll('.ranking-book-check').forEach(cb => {
      cb.addEventListener('change', () => {
        const id = cb.getAttribute('data-book-id');
        if (cb.checked) {
          st.selected.add(id);
        } else {
    ...
    // 绑定单个下载按钮
    tbody.querySelectorAll('.ranking-download-single').forEach(btn => {
      btn.addEventListener('click', async () => {
        const bookId = btn.getAttribute('data-book-id');
        const title = btn.getAttribute('data-title');
        await submitDownloadJob(bookId, title);
      });
    });

    // 更新分页
    const total = data.total || data.items.length;
    const totalPages = Math.ceil(total / (data.size || 50));
    updateRankingPagination(channelId, st.page, totalPages, total);

    // 更新全选框
    updateRankingSelectAll(channelId);

  } catch (err) {
    console.error('loadRankingBooks:', err);
    tbody.innerHTML = `<tr><td colspan="6" class="text-center pad-20 error">加载失败: ${escHtml(err.message || err)}</td></tr>`;
  }
}

function updateRankingPagination(channelId, page, totalPages, total) {
  const prevBtn = document.getElementById(`rankingPrev${channelId}`);
  const nextBtn = document.getElementById(`rankingNext${channelId}`);
  const info = document.getElementById(`rankingPageInfo${channelId}`);

  if (prevBtn) prevBtn.disabled = page <= 1;
  if (nextBtn) nextBtn.disabled = page >= totalPages;
  if (info) info.textContent = totalPages > 0
    ? `第 ${page} / ${totalPages} 页，共 ${total} 本`
    : '';
}

function updateRankingSelectedCount(channelId) {
  const st = rankingState[channelId];
  const count = st.selected.size;
  const el = document.getElementById(`rankingSelectedCount${channelId}`);
  const btn = document.getElementById(`rankingBatchDownload${channelId}`);

  if (el) el.textContent = `已选 ${count} 本`;
  if (btn) btn.disabled = count === 0;

  updateRankingSelectAll(channelId);
}

function updateRankingSelectAll(channelId) {
  const st = rankingState[channelId];
  const tbody = document.getElementById(`rankingBooksBody${channelId}`);
  const selectAll = document.getElementById(`rankingSelectAll${channelId}`);
  if (!tbody || !selectAll) return;

  const checks = Array.from(tbody.querySelectorAll('.ranking-book-check'));
  if (checks.length === 0) { selectAll.checked = false; return; }
...
function setupRankingChannelEvents(channelId) {
  // 全选
  const selectAll = document.getElementById(`rankingSelectAll${channelId}`);
  if (selectAll) {
    selectAll.addEventListener('change', () => {
      ...
      if (selectAll.checked) {
        checks.forEach(cb => {
          cb.checked = true;
          st.selected.add(cb.getAttribute('data-book-id'));
        });
      } else {
        checks.forEach(cb => {
          cb.checked = false;
          st.selected.delete(cb.getAttribute('data-book-id'));
        });
      }
      updateRankingSelectedCount(channelId);
    });
  ...
  // 批量下载
  const batchBtn = document.getElementById(`rankingBatchDownload${channelId}`);
  if (batchBtn) {
    batchBtn.addEventListener('click', async () => {
      const st = rankingState[channelId];
      if (st.selected.size === 0) return;
      await batchDownloadRanking(channelId);
    });
  }
  ...
  if (prevBtn) {
    prevBtn.addEventListener('click', () => {
      const st = rankingState[channelId];
      if (st.page > 1) { st.page--; loadRankingBooks(channelId); }
    });
  }
  if (nextBtn) {
    nextBtn.addEventListener('click', () => {
      const st = rankingState[channelId];
      st.page++; loadRankingBooks(channelId);
    });
  }
}

async function batchDownloadRanking(channelId) {
  const st = rankingState[channelId];
  const total = st.selected.size;
  if (total === 0) return;

  const confirmMsg = `确定要批量下载已选的 ${total} 本书吗？`;
  if (!confirm(confirmMsg)) return;

  // 获取所有选中的书籍信息（从当前页的 DOM 中提取）
  const tbody = document.getElementById(`rankingBooksBody${channelId}`);
  const selectedBooks = [];
  tbody.querySelectorAll('.ranking-book-check:checked').forEach(cb => {
    selectedBooks.push({
      book_id: cb.getAttribute('data-book-id'),
  ...
  for (const book of selectedBooks) {
    const ok = await submitDownloadJob(book.book_id, book.title);
    if (ok) success++;
    else failed++;
    // 间隔 500ms 避免请求过快
    await sleep(500);
  ...
  alert(`批量下载任务已提交：成功 ${success} 本，失败 ${failed} 本`);
  st.selected.clear();
  updateRankingSelectedCount(channelId);
  // 跳转到任务列表
  window.location.hash = '#jobs';
...
async function submitDownloadJob(bookId, title) {
  try {
    const resp = await fetchWithCreds('/api/jobs', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ book_id: bookId }),
    });
    if (resp.status === 429) {
      alert('同时只能有 1 个下载任务，请等待当前任务完成后再试');
      return false;
    }
    if (!resp.ok) {
      const err = await resp.json().catch(() => ({}));
      alert(`创建下载任务失败: ${err.error || resp.statusText}`);
      return false;
    }
    return true;
  } catch (err) {
    console.error('submitDownloadJob:', err);
